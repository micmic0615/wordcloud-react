                ...
                if (JSON.stringify(compareA) !== JSON.stringify(compareB)){
                    DrawCanvas.bind(this)(GenerateConfig.bind(this)({
                        ...
                        color: (word, weight, fontSize, distance, theta, info) => {
                            ...
                            let redistance = Math.pow(distance, 0.5)
                            let red = 255 - (150*redistance)
                            let green = (135*redistance) + 120;
                            let background =  `rgb(${red}, ${green}, 0)`

                            return (!this.baseList.map(p=>p[0]).includes(word) ? emphasis : background);
                        },
                    }))
